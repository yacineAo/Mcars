...
**Status: ⬜** · Depends on: Phases 4, 5 · Closes: **REQ-07**, **REQ-14**, **REQ-15**, **REQ-03**
(payment side), **ADV-07**
...
- [ ] `owner_installments`
- [ ] `fines`
- [ ] `employees`, `payroll_runs`, `payroll_items`, `employee_advances`, `commissions`

### Enums
- [ ] `PaymentMethod` — `cash`, `bank_transfer`, `ccp`, `card`, `baridimob`, `cheque`,
...
- [ ] `PaymentDirection`, `PaymentStatus`, `PaymentScheduleStatus`, `DepositStatus`,
...
- [ ] `PaymentPoster` · `DepositPoster` · `OwnerInstallmentPoster` · `FinePoster` · `PayrollPoster`

### Services
- [ ] **`DepositService`** — hold / deduct / refund / forfeit. **A deposit is a liability, never
      ...
      lives in one place.
- [ ] **`OwnerStatementService`** — monthly instalment generation from active agreements (scheduled),
      payment recording. **Owner remaining balance is the balance of account 2200 filtered by
      `car_owner_id`** — never a column.
- [ ] **`FineLiabilityService`** — match `violation_at` against contracts active for that car at that
...
### Resources
- [ ] `PaymentResource`, `PaymentScheduleResource` (overdue filter), `DepositResource` (+ deduction
...
- [ ] Un-stub the Financials and Fines tabs on `CustomerResource`
...
- [ ] Every remaining posting-matrix row
- [ ] A held deposit **never** appears in revenue
- [ ] Deductions cannot exceed the deposit; the overflow lands in receivables
- [ ] Owner balance = accrued − paid, read from account 2200
- [ ] A customer-liable fine leaves profit unchanged until written off
- [ ] An advance is an asset, not an expense
- [ ] Sum of partial payments equals the invoice and the derived balance reaches **exactly** zero
- [ ] An instalment plan of 100.00 over 3 totals exactly 100.00 (`Money::allocate`)
