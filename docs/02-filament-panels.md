...
| `FinancialAccountResource` | Cash boxes, bank, CCP, BaridiMob, POS. Index has derived `current_balance` column gated on `reports.view_financials` (batched via `CashRegisterService::balancesBatch()`), filters by type/is_active/branch. View page with account details, bank details (conditional on type), two header actions gated on `finance.manage_capital` — Record Capital Injection (E70) and Record Owner Drawing (E71), both equity postings via `CashRegisterService` — and two read-only relation managers: Transactions (both legs) and CashSessions. The `opening_balance` field on the form is descriptive only — it is never read into `current_balance` or any derivation, so it must never be used to represent real capital; that is what the two header actions are for. |
